  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      {loading ? (
        ...
        <div className="bg-paper-100 border border-paper-200 rounded-2xl p-12 text-center text-ink-700">
          <FileText className="w-10 h-10 text-ink-700/40 mx-auto mb-3" />
          <h3 className="text-base font-semibold text-ink-900">No saved drafts</h3>
          <p className="text-xs text-ink-700 mt-1">Drafts automatically save every ~10 seconds while composing.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {drafts.map((d) => (
            <div
              key={d.id}
              className="bg-paper-100 border border-paper-200 hover:border-accent-400 rounded-2xl p-5 shadow-sm flex flex-col justify-between transition-all"
            >
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-amber-500/10 text-amber-800 border border-amber-500/30">
                    {d.status}
                  </span>
                ...
                <p className="text-xs text-ink-700 line-clamp-3 leading-relaxed">
                  {d.body}
                </p>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-paper-200 flex items-center justify-between">
                <button
                  ...
                  className="text-xs text-accent-500 font-semibold hover:underline flex items-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="p-1.5 text-ink-700 hover:text-rose-600 transition-colors"
                  title="Delete Draft"
                >
